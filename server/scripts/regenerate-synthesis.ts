/**
 * Regenerate Synthesis from existing analysis files
 *
 * Usage: npx tsx scripts/regenerate-synthesis.ts [prompt]
 *
...
function stripThinkingText(content: string, marker: string | RegExp): string {
  if (typeof marker === 'string') {
    ...
    if (index > 0) {
      console.log(`[RegenSynthesis] Stripping ${index} chars of thinking text`);
      return content.slice(index);
    }
  } else {
    const match = content.match(marker);
    if (match && match.index && match.index > 0) {
      console.log(`[RegenSynthesis] Stripping ${match.index} chars of thinking text`);
...
async function regenerateSynthesis(userPrompt?: string) {
...
regenerateSynthesis(userPrompt).catch(console.error);
